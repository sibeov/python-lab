...
def check_version(version_string_: str, required_version_: str = "18.1.1") -> None:
    if not version_string_ == required_version_:
        raise QuartusVersionError(
            f"Quartus Version {version_string_} does not match required version {required_version_}"
        )
...
async def main() -> int:
    ...
    async with asyncio.TaskGroup() as task_group:
        ...
        t1_results = await t1
        t2 = task_group.create_task(
            get_quartus_map_version_from_result(t1_results.stdout)
        )
        t2_results = await t2
